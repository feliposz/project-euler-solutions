...
"""
from eulerlib import reverseNum, isNumPalindrome

def isLycrel(num, limit=50):
    """Check if number is "theoretically" a lycrel number."""
    count = 0
    while count < limit:
        count += 1
        nextNum = num + reverseNum(num)
        #print("i =", count, "num =", num, "rev =", reverseNum(num), "next =", nextNum)
        if isNumPalindrome(nextNum):
            return False
        num = nextNum
    return True
        
def test55():
    for n in [47, 349, 10677, 196]:
        print(n, isLycrel(n))

def euler55():
    """Solves problem 55 from Project Euler."""
    count = 0
    for n in range(1, 10000):
        ...
        if test:
            print(n, test)
            count += 1
    print("Lycrel numbers found =", count)
